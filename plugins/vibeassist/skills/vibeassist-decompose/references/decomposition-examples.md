...
acceptance. The tree stays three cards deep at most, and every card is a
...
never become child cards. A decomposition that turns each behaviour into a
sub-card has mistaken specification for structure: the map gets deep and
unreadable, and every card stops being a capability.

When you catch yourself writing a child card whose name is a property, a
...
A card earns its place by being **recognizable from its name alone**.
...
- Wanting to group things is fine — that's what **tags** are for. Tags are
  lenses, never places: tags may be categories; card names may not.
...
NOT quietly add a "Password reset" card (inventing structure the code doesn't
...
> planned ask?
> **Recommended: yes, as a held/proposed card** — users will expect it, and
...
The tree is agreed and _Task list_ is a card. Its shaping pass captures the
...
Each confirmed answer becomes an **acceptance checklist item** on the card's
...
card can be deliberately **held** as proposed — visible on the map, part of
...
don't count toward a parent's readiness; releasing the hold returns the card
to the normal ladder. MVP scoping lives ON the map, never in a separate
backlog — so when a walk surfaces a "later" want, propose it as a held card
rather than dropping it.

## Live is fact, not desire — recompose, don't erase

In breakdown mode you judge whether a capability **exists and runs** — never
whether the user still wants it. A map of a real app will contain live cards
the user intends to reshape or retire; that is expected, not an error. The
ladder has a re-entry path: a live card can be deliberately sent back to
