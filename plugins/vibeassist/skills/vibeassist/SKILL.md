...
<!-- 0.28.0 (24 Aug 2026): a `shape_ask` job is now the WHOLE shaping conversation — Form and Confirm in one talk, one question at a time through `ask_user`, no verdict and no list handed back — and it ends with the read-back, which lands as the build notes. `check_shape` is retired: a stray one is run as the Confirm movement and finished with `report_shape_review({ jobId })` carrying no findings. `report_build_notes` takes `jobId`, not `askId`. See references/standby.md. -->
