...
<!-- 0.52.0 (5 Sep 2026): NEW TOOL `say_in_chat({ jobId, said })` — tell the person one line in the ask's chat (a tell, not a question; nothing parks). Added to the tools table (§ 1). It carries the shaping DELTA and the build-notes HANDOFF now that questions, answers and the read-back all live in the chat; the how-and-when is in the decompose skill (§ the shaping conversation, § the plan) and references/standby.md (the shape_ask and write_build_notes descriptions). No change to how this skill builds. -->
<!-- 0.51.0 (12 Aug 2026): version synced to the plugin bundle (0.51.0 lands set_components + the build-notes-may-ask change on the decompose skill). No change to this skill; the marker moves to keep every skill in step with plugin.json, which the version check requires. -->
<!-- 0.48.0 (4 Sep 2026): cd IS ALLOWED TO GET INTO THE WORKTREE — the fix 0.46/0.47 were reaching for. Banning cd outright did not remove the stall, it MOVED it: with cd forbidden, a build sub-agent that needs a real working directory (install, typecheck, test, dev build — npm/vite/tsc want a cwd, and `git -C`/`--prefix` don't give one) reached for `EnterWorktree` instead, whose permission-root relocation to a worktree outside `.claude/worktrees/` is an approval that never comes unattended — same hang, new doorway. Root cause was never cd itself: it was that the checkout's allow-list listed `git`/`npm`/`npx`/`node` but NOT `cd`/`grep`/`ls`/`cat`/`find`, so any of those (with or without a cd) fell to a prompt. Fix is two-part: (1) add `cd` + the read/nav commands to `.claude/settings.local.json` allow (per checkout, one-off, machine-local); (2) references/standby.md § Resolving it, per job now SPLITS the rule — git and file-search still NAME THEIR DIRECTORY (`git -C`, for mainline-from-served-checkout correctness, not just the prompt), but running the build cd's into the worktree, and `EnterWorktree` is banned outright as the one gate no setting can suppress. Supersedes the absolute "never cd" of 0.47.0. -->
<!-- 0.47.0 (3 Sep 2026): THE "NAME THE DIRECTORY, NEVER cd" RULE COVERS EVERY COMMAND, NOT JUST GIT. 0.46.0 fixed `cd <worktree> && git …` but a general-purpose sub-agent then hit the same wall with `cd <worktree> && grep …` (searching for an implementation). ANY file-reading command after a cd has an unresolvable working directory, which the `.env` deny floor turns into an approval prompt that never comes unattended. references/standby.md § Resolving it, per job now says EVERY command names its directory — git -C, grep <wt>/src, find <wt>, cat <wt>/… — never a cd, and the SPAWNING agent must brief its sub-agents with the worktree path and this rule, because a delegated general-purpose agent does not read the plugin references. Known limit: this reduces but cannot fully guarantee a freelancing sub-agent's behaviour; the airtight stop is the permission level (the .env deny), left in place because it guards other projects' real secrets. -->
<!-- 0.46.0 (3 Sep 2026): GIT IN A WORKTREE NAMES THE DIRECTORY — never a cd into it. Superseded/widened by 0.47.0 (every command, not just git). -->
<!-- 0.46.0 (3 Sep 2026): GIT IN A WORKTREE NAMES THE DIRECTORY — never a cd into it, and never in a delegated sub-agent. references/standby.md § Resolving it, per job used to allow git "from inside the worktree made under it"; a fresh code_check drift agent took that literally and ran `cd <worktree> && git diff types.gen.ts`. A bare cd leaves the path unresolvable to the permission check, so with the recommended deny floor (`.env`) in place it could not prove the diffed path safe and STALLED on an approval prompt — which never comes when running unattended, so the job hangs (fatal to a headless/fleet worker). Now the convention is `git -C <where> …` / `git -C <checkout>-<shortId> …` always, cd-into-a-folder-then-git is banned outright, and the code_check drift bullet names the form. See references/standby.md § Resolving it, per job and references/code-check.md § 5. -->
<!-- 0.45.0 (3 Sep 2026): CHECKS RUN THEIR OWN SERVER — NEVER THE OWNER'S. The review's "run it" (references/review.md § 4) and the build's verify-inward (references/delivery-on-asks.md § 7) have to run the app, and were reaching for the one shared dev server — the owner's, on port 3000. Many checks plus the owner all on one server is what bottlenecks and crashes it. Now a check runs its OWN instance in its OWN worktree on its OWN port, never port 3000, and kills it when done — build-isolation extended from building to verifying. The skill stays command-agnostic; the concrete serve command lives in the app's CLAUDE.md § Serving it for a review (`vite dev` on a free port here). -->
<!-- 0.44.0 (3 Sep 2026): A RESOLVED MUST-NOT CONTRADICTION HEALS THE SHAPE — it never just clears one build. Primary catch is pre-build, in shaping (decompose skill § Record a change only when the shape's own words go wrong). This is the build-side BACKSTOP: if a build meets a must-not its own delivery contradicts, it surfaces the exact replacement wording via ask_user and the line is rewritten on confirm, and it NEVER delivers an ask whose must-not still fights what it built — an unchanged line is re-found by every future build and review, which is a nag-loop, not a resolution. If the tool refuses to rewrite an owner-agreed line, that is BLOCKING, not done. See references/delivery-on-asks.md § the plan never outranks a must-not. APP DEPENDENCY: the mid-build heal needs the app to let an owner-confirmed contradiction rewrite the agreed line; until that lands the backstop flags blocking rather than silently re-delivering, and the pre-build shaping catch (skill-only, executable now) is what actually closes the loop. -->
<!-- 0.43.0 (3 Sep 2026): REAP THE WORKTREES A MERGE NEVER GOT TO. Cleanup-on-merge (the reviewer removes the worktree on a pass, 0.30.0) only clears worktrees whose build reached the merge. A build that failed its rounds and stopped for a person, a dead session, an ask dropped/deleted, or a board reconciled out from under it, leaves its worktree behind with nobody coming for it — one per build, for ever (~45 had piled up, the pile that hid the stranded work). New at references/standby.md § Before you arm — reap the worktrees that are done with: a once-per-kickoff sweep (never inside the loop) removes any build worktree beside the served checkout — tracked OR git-forgotten — but ONLY when its ask has no live job AND (its branch is merged into the main line OR its ask is gone/dropped), never the served checkout, unlinking a node_modules symlink before deleting a forgotten folder so a recursive delete can't follow it into the shared store. Silent and conservative: a doubtful worktree is always left, never forced. The build-side rule is unchanged — a build still LEAVES its worktree as the handoff; this only clears the ones the merge never reached. -->
...
| Tell the person a line in chat | `say_in_chat({ jobId, said })`                  |
